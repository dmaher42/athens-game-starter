...
function normalizeSettings(partial = {}) {
    ...
    const normalized = {
        ...
        yawSpeed: clamp(toNumber(merged.yawSpeed, defaultCameraSettings.yawSpeed), CAMERA_RANGES.yawSpeed.min, CAMERA_RANGES.yawSpeed.max),
        pitchSpeed: clamp(toNumber(merged.pitchSpeed, defaultCameraSettings.pitchSpeed), CAMERA_RANGES.pitchSpeed.min, CAMERA_RANGES.pitchSpeed.max),
        zoomSpeed: clamp(toNumber(merged.zoomSpeed, defaultCameraSettings.zoomSpeed), CAMERA_RANGES.zoomSpeed.min, CAMERA_RANGES.zoomSpeed.max),
        minPitch: clamp(toNumber(merged.minPitch, defaultCameraSettings.minPitch), CAMERA_RANGES.minPitch.min, CAMERA_RANGES.minPitch.max),
        maxPitch: clamp(toNumber(merged.maxPitch, defaultCameraSettings.maxPitch), CAMERA_RANGES.maxPitch.min, CAMERA_RANGES.maxPitch.max),
        minDist: clamp(toNumber(merged.minDist, defaultCameraSettings.minDist), CAMERA_RANGES.minDist.min, CAMERA_RANGES.minDist.max),
        maxDist: clamp(toNumber(merged.maxDist, defaultCameraSettings.maxDist), CAMERA_RANGES.maxDist.min, CAMERA_RANGES.maxDist.max),
    ...
    if (normalized.minDist === normalized.maxDist) {
        if (normalized.maxDist < CAMERA_RANGES.maxDist.max) {
            normalized.maxDist = Math.min(CAMERA_RANGES.maxDist.max, normalized.maxDist + 0.1);
        }
        else {
            normalized.minDist = Math.max(CAMERA_RANGES.minDist.min, normalized.minDist - 0.1);
        }
    }
