...
export function mountDevHUD(options = {}) {
    const { getPosition, getDirection, onPin, onSetLightingPreset, lightingPresets, getFogEnabled, onToggleFog, } = options;
    ...
    const setOceanStatus = (options = {}) => {
        ...
        const boundsAreValid = !!bounds && boundKeys.every((key) => Number.isFinite(bounds?.[key]));
        if (!levelIsFinite || !boundsAreValid) {
            setStatusLine("sea", "");
            return;
        }
        const safeBounds = bounds;
        const formatBound = (value) => value.toFixed(1);
        const message = [
            `Sea level: ${Number(seaLevel).toFixed(2)}`,
            `Ocean bounds: W ${formatBound(safeBounds.west)} / E ${formatBound(safeBounds.east)} / N ${formatBound(safeBounds.north)} / S ${formatBound(safeBounds.south)}`,
        ].join("\n");
    ...
    const onKey = (e) => {
        ...
        if (!bindings || typeof onSetLightingPreset !== "function")
            return;
        if (e.repeat)
            return;
