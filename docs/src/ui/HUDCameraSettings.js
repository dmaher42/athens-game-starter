...
function createCheckbox(labelText, key, onChange) {
    ...
    return { wrapper: wrapper, checkbox };
}
export function mount(rootEl) {
    ...
    const enableCheckbox = createCheckbox("Enable Arrow Orbit", "enableArrowOrbit", (key, checked) => {
        if (state.disposed)
            return;
        const update = { [key]: checked };
        saveSettings(update);
    });
    const invertCheckbox = createCheckbox("Invert Pitch", "invertPitch", (key, checked) => {
        if (state.disposed)
            return;
        const update = { [key]: checked };
        saveSettings(update);
