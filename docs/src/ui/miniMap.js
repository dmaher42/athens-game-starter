import { ACROPOLIS_PEAK_3D, AGORA_CENTER_3D, CITY_AREA_RADIUS, CITY_CHUNK_CENTER, HARBOR_CENTER_3D, } from "../world/locations.js";
...
function buildLandmarkFeatures() {
    ...
    for (const group of groups) {
        if (!group || !Array.isArray(group.landmarks))
            continue;
...
export function mountMiniMap(options = {}) {
    if (typeof document === "undefined")
        return null;
    ensureStyles();
    const { getPosition = () => null, getDirection = () => ({ x: 0, z: 1 }), features = DEFAULT_FEATURES, } = options;
